...
public class PayrollReader {
    ...
    static void main() {
        ...
        PayrollReader pr = new PayrollReader(fileName);

        while (pr.getNextRecord()) {
            String name = pr.nameOf();
            int hours = pr.hoursOf();
            int payrate = pr.payrateOf();
            int payment = hours * payrate;
